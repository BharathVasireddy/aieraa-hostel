...
const mockDataFiles = [
  {
    ...
    patterns: [
      {
        search:
          /\/\/ Mock popular items and category breakdown for speed[\s\S]*?const categoryBreakdown = \[[\s\S]*?\]/,
    ...
    const categoryBreakdown = await prisma.orderItem.groupBy({
      ...
      },
      {
        ...
        replace:
          'revenueGrowth: 0, // Real growth calculation would require historical data',
      },
      {
        search: /orderGrowth: 8\.3, \/\/ Mock growth rate for speed/,
        replace:
          'orderGrowth: 0, // Real growth calculation would require historical data',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        replace:
          'orders: 0, // Real order count will be populated from actual orders',
      },
      {
        search:
  ...
  },
  {
    ...
    patterns: [
      ...
      },
      {
        ...
        replace:
          'customerSatisfaction: 0 // Will be calculated from real feedback',
      },
    ],
  },
];
...
async function cleanupOrdersData() {
  ...
  console.log(
    '⚠️  This will permanently delete ALL order data from the database!'
  );
  console.log('');
  ...
  try {
    ...
    console.log(
      `   - Menu Item Availability with quantities: ${availabilityCount}`
    );
    console.log('');
