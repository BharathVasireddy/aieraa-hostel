  ...
  const fetchData = useCallback(async (isRetry = false) => {
    ...
    try {
      ...
      if (attemptCountRef.current <= retryCount) {
        console.warn(`Progressive loading attempt ${attemptCountRef.current} failed, retrying...`, error)
...
export function useProgressiveMultiLoading<T extends Record<string, any>>(
  ...
  const hasAnyErrors = loaderKeys.some(key => loadingStates[key].error !== null)
