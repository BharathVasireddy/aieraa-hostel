    ...
    (
      ...
      useEffect(() => {
            ...
            entries.forEach(entry => {
              if (entry.isIntersecting) {
                const index = parseInt(
                  entry.target.getAttribute('data-prefetch-index') || '0'
                );
        ...
        return () => {
          ...
          // Clean up elements
          document
            .querySelectorAll('[data-prefetch-index]')
...
export function useSmartPrefetch() {
  ...
  const prefetchUserFlow = useCallback(
    (flow: string[]) => {
...
export function useHostelPrefetch() {
  ...
  const prefetchByRole = useCallback(
    (role: string) => {
      switch (role) {
        case 'STUDENT':
          prefetchStudentFlow();
  ...
  const prefetchTomorrowMenu = useCallback(
    (universityId: string) => {
