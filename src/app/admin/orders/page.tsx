...
export default function AdminOrders() {
  ...
  const filterOrders = useCallback(() => {
    ...
    if (currentUserData?.role === 'ADMIN' && selectedUniversity !== 'all') {
      filtered = filtered.filter(
        order => order.university?.id === selectedUniversity
      );
    }
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      const ordersResponse = await fetch(
        `/api/admin/orders?university=${selectedUniversity}&status=${selectedTab}&page=1&limit=100`,
        {
          headers: {
            'Cache-Control': 'no-cache, no-store, must-revalidate',
            Pragma: 'no-cache',
  ...
  const updateOrderStatus = async (
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast({
          type: ['REJECTED', 'CANCELLED'].includes(newStatus)
            ? 'error'
            : 'success',
          title: ['REJECTED', 'CANCELLED'].includes(newStatus)
            ? 'Order Rejected'
            : 'Status Updated',
          message:
  ...
  const filterTabs = [
    ...
    {
      key: 'PREPARING' as FilterStatus,
      label: 'Preparing',
      color: 'purple',
      count: orders.filter(o => o.status === 'PREPARING').length,
    },
    {
      key: 'READY' as FilterStatus,
      label: 'Ready',
      color: 'green',
      count: orders.filter(o => o.status === 'READY').length,
    },
    {
      key: 'SERVED' as FilterStatus,
      label: 'Served',
      color: 'emerald',
                          ...
                          >
                            <div className='flex items-center justify-between'>
                              ...
                              <span className='text-xs text-gray-500'>
                                {
                                  orders.filter(
                                    o => o.university?.id === university.id
                                  ).length
                                }
                              </span>
                            </div>
        ...
        <div className='max-w-7xl mx-auto'>
          ...
          {loading ? (
            ...
            <>
              {/* Desktop List View */}
              <div
                className={`hidden lg:block ${viewMode === 'list' ? '' : 'lg:hidden'}`}
              >
                <div className='bg-white rounded-lg border overflow-hidden'>
                  ...
                  <div className='bg-gray-50 px-6 py-3 border-b'>
                    <div className='grid grid-cols-8 gap-4 text-sm font-medium text-gray-700'>
                      ...
                      <div
                        className={
                          currentUserData?.role === 'ADMIN' ? 'col-start-5' : ''
                        }
                      >
                        Status
                      </div>
                    ...
                    {filteredOrders.map(order => (
                      ...
                      >
                        <div className='grid grid-cols-8 gap-4 items-center text-sm'>
                              ...
                              {order.orderItems
                                ...
                                .map((item, index) => (
                                  <span key={item.id}>
                                    ...
                                    {index <
                                      order.orderItems.slice(0, 2).length - 1 &&
                                      ', '}
                                  </span>
                                ))}
                          ...
                          >
                            <div
                              className={`inline-flex items-center space-x-2 px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(order.status)}`}
                            >
                        ...
                        {!['REJECTED', 'CANCELLED', 'SERVED'].includes(
                          order.status
                        ) && (
                          <div className='mt-3 pt-3 border-t border-gray-100'>
                            ...
                            <div className='flex items-center space-x-2'>
                              <div
                                className={`w-3 h-3 rounded-full ${order.status === 'PENDING' ? 'bg-orange-500' : 'bg-gray-300'}`}
                              ></div>
                              <ArrowRight className='w-3 h-3 text-gray-400' />
                              <div
                                className={`w-3 h-3 rounded-full ${order.status === 'APPROVED' ? 'bg-blue-500' : 'bg-gray-300'}`}
                              ></div>
                              <ArrowRight className='w-3 h-3 text-gray-400' />
                              <div
                                className={`w-3 h-3 rounded-full ${order.status === 'PREPARING' ? 'bg-purple-500' : 'bg-gray-300'}`}
                              ></div>
                              <ArrowRight className='w-3 h-3 text-gray-400' />
                              <div
                                className={`w-3 h-3 rounded-full ${order.status === 'READY' ? 'bg-green-500' : 'bg-gray-300'}`}
                              ></div>
                            </div>
                          </div>
                        )}
              ...
              <div
                className={`lg:${viewMode === 'cards' ? 'block' : 'hidden'} lg:grid-cols-2 xl:grid-cols-3 gap-6 grid grid-cols-1`}
              >
                {filteredOrders.map(order => (
                  <div
                    key={order.id}
                  ...
                  >
                    ...
                    <div className='flex items-center justify-between mb-4'>
                      <div
                        className={`flex items-center space-x-2 px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(order.status)}`}
                      >
                        {getStatusIcon(order.status)}
                        <span>{order.status}</span>
                      </div>
                    ...
                    {currentUserData?.role === 'ADMIN' && order.university && (
                      <div className='flex items-center space-x-2 mb-4 p-2 bg-gray-50 rounded-lg'>
                        ...
                        <span className='text-sm text-gray-700'>
                          {order.university.name} ({order.university.code})
                        </span>
                      </div>
                    )}
                    ...
                    <div className='space-y-3'>
                      ...
                      <div className='flex space-x-2'>
                        ...
                        {!['SERVED', 'REJECTED', 'CANCELLED'].includes(
                          order.status
                        ) && (
                          <button
                            onClick={() => cancelOrder(order.id)}
