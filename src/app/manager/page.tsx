...
export default function ManagerDashboard() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/manager/analytics/stats?universityId=${user.universityId}`, {
        headers: {
  ...
  const fetchRecentOrders = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/manager/orders/recent?universityId=${user.universityId}&limit=5`)
  ...
  const fetchPopularItems = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/manager/analytics/popular-items?universityId=${user.universityId}&limit=4`)
      ...
      <div className="px-4 py-6 space-y-6">
        ...
        <div className="bg-gradient-to-r from-green-600 to-blue-600 rounded-2xl p-6 text-white">
          ...
          <p className="text-green-100">
            Managing {user.university?.name || 'University'} • {new Date().toLocaleDateString()}
          </p>
        </div>
