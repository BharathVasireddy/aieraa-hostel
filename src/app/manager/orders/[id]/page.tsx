...
export default function ManagerOrderDetailsPage() {
  ...
  const updateOrderStatus = async (status: 'APPROVED' | 'PREPARING' | 'READY' | 'SERVED' | 'CANCELLED') => {
    if (!order) return
              ...
              {order.user.studentId && (
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{order.user.studentId}</p>
                  </div>
                </div>
              )}
              {order.user.roomNumber && (
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{order.user.roomNumber}</p>
                  </div>
                </div>
              )}
              {order.user.phone && (
            ...
            <div className="space-y-3">
              ...
              {order.status === 'APPROVED' && (
                <>
                  ...
                  >
                    {updating ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Clock className="w-4 h-4" />}
              ...
              {order.status === 'PREPARING' && (
                <>
                  ...
                  >
                    {updating ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Package className="w-4 h-4" />}
              ...
              {(order.status === 'SERVED' || order.status === 'CANCELLED') && (
                <div className="text-center py-4">
                  <p className="text-gray-600">Order is {order.status.toLowerCase()}</p>
