...
export async function GET(request: NextRequest) {
  try {
    ...
    const dashboard = {
      ...
      health: apiHealth,
      summary: {
        status: performanceData.stats.averageResponseTime < 200 ? 'excellent' : 
                performanceData.stats.averageResponseTime < 500 ? 'good' : 'needs_attention',
        score: calculatePerformanceScore(performanceData.stats),
        criticalIssues: performanceData.insights.filter(insight => 
          insight.includes('⚠️')
        ).length
      }
    }
