...
export async function POST(
  ...
  try {
    ...
    if (validatedData.mode === 'recurring' && validatedData.recurringPattern) {
      ...
      validatedData.mode === 'specificDates' &&
      validatedData.specificDates
    ) {
      datesToSet = validatedData.specificDates.dates.map(
    ...
    // Log the action
    console.log(
      `Manager ${currentUser.name} set availability for ${menuItem.name}: ${validatedData.mode} mode, ${result} dates`
    );
...
function generateRecurringDates(
  pattern: z.infer<typeof RecurringPatternSchema>
): Date[] {
  const dates: Date[] = [];
...
function generateDateRangeDates(
