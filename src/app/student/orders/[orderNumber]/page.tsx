...
export default function StudentOrderDetail({
  ...
  const fetchOrderDetail = async () => {
    try {
      ...
      console.log(
        'Fetching order details for order number:',
        orderNumber,
        'User ID:',
        session?.user?.id
      );
  ...
  const getStatusMessage = (status: string) => {
    switch (status) {
      ...
      case 'READY':
        return 'Your order is ready for pickup! Show your QR code to the counter staff';
      case 'SERVED':
      ...
      <div className='px-4 py-6 space-y-6'>
        ...
        <div className='bg-white rounded-lg p-6 border border-gray-100'>
          <div className='flex items-center justify-between mb-4'>
            <div
              className={`flex items-center space-x-2 px-3 py-2 rounded-full text-sm font-medium border ${getStatusColor(order.status)}`}
            >
          ...
          <div className='space-y-4'>
            <div className='flex items-start space-x-3'>
              ...
              <div>
                ...
                <p className='text-sm text-gray-600'>
                  {formatVietnamDateTime(new Date(order.createdAt))}
                </p>
              </div>
            </div>

            {['APPROVED', 'PREPARING', 'READY', 'SERVED'].includes(
              order.status
            ) && (
              <div className='flex items-start space-x-3'>
            ...
            {order.status === 'SERVED' && order.completedAt && (
              <div className='flex items-start space-x-3'>
                ...
                <div>
                  ...
                  <p className='text-sm text-gray-600'>
                    {formatVietnamDateTime(new Date(order.completedAt))}
                  </p>
                </div>
              </div>
            )}
          ...
          <div className='bg-white rounded-lg p-6 max-w-sm w-full'>
            <div className='text-center'>
              ...
              <p className='text-sm text-gray-600 mb-6'>
                Present this QR code to the counter staff to collect your order
              </p>
