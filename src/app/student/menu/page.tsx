...
export default function StudentMenu() {
    ...
    if (savedCart) {
      try {
        ...
        if (cartData.orderDate === selectedDate) {
          ...
          const items: CartItem[] = Object.entries(cartData.items).map(
            ([id, quantity]) => ({
              id,
  ...
  const getImageUrl = useCallback((itemName: string, category: string) => {
    const foodImages: { [key: string]: string } = {
      ...
      'chicken biryani':
        'https://images.unsplash.com/photo-1563379091339-03246a1d0220?w=200&h=200&fit=crop',
      'dal tadka':
  ...
  return (
    <StudentLayout className='pb-16'>
        ...
        <div className='border-b border-gray-100 px-4 py-3'>
          <div className='flex space-x-3'>
            ...
            <div className='flex items-center space-x-2 px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl'>
              <Leaf
                className={`w-4 h-4 transition-colors duration-200 ${showVegOnly ? 'text-green-600' : 'text-gray-400'}`}
              />
          ...
          <div className='px-4 py-3 bg-red-50 border-b border-red-200'>
            <div className='text-center'>
              <div className='text-sm text-red-800'>
                <strong>Ordering closed for this date.</strong> Orders must be
                placed before 10:00 PM Vietnam time the day before.
              </div>
      ...
      <div
        ...
        style={{ paddingBottom: cartItems.length > 0 ? '140px' : '80px' }}
      >
        {filteredItems.length === 0 ? (
          <div className='text-center py-8'>
            ...
            <p className='text-sm text-gray-600'>
              {searchQuery
                ...
                : showVegOnly
                  ? 'Try turning off the veg filter'
          ...
          filteredItems.map(item => {
            ...
            return (
              <div
                key={item.id}
                className='bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-200 hover:scale-[1.02] active:scale-[0.98]'
              >
                <div className='flex'>
                  {/* Food Image */}
                        ...
                        <div className='flex items-center justify-between'>
                          ...
                          {quantityInCart > 0 ? (
                            ...
                            >
                              <Plus className='w-4 h-4' />
                              <span>
                                {countdown.isPastCutoff ? 'Closed' : 'Add'}
                              </span>
                            </button>
                          )}
                        </div>
        ...
        <div className='relative'>
          ...
          >
            <div className='flex flex-col items-center'>
              ...
              <Menu
                className={`w-3 h-3 text-gray-600 mt-0.5 transition-transform duration-200 ${showCategoryMenu ? 'rotate-180' : ''}`}
              />
            </div>
          </button>
                  ...
                  const count =
                    category.key === 'all'
                      ...
                      : menuItems.filter(item => {
                          const matchesCategory =
                            item.category.toLowerCase() === category.key;
                          const matchesSearch =
                            item.name
                              .toLowerCase()
                          ...
                          const matchesVegFilter =
                            !showVegOnly || item.isVegetarian;
                          return (
                            matchesCategory && matchesSearch && matchesVegFilter
                          );
                        }).length;
      ...
      {showVariationSheet && selectedItem && (
        <div
          className='fixed inset-0 z-60 bg-black bg-opacity-50 animate-in fade-in duration-200'
          onClick={() => {
            setShowVariationSheet(false);
            setSelectedItem(null);
          }}
