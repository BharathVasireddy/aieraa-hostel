...
export async function lightningFastFetch(
  url: string,
  options?: RequestInit
): Promise<any> {
...
// Higher-order function for tracking API endpoints
export function trackAPIEndpoint(endpoint: string) {
  return function <T>(apiCall: () => Promise<T>): Promise<T> {
    return withPerformanceMonitoring(endpoint, apiCall);
  };
}
...
export function getPerformanceSummary(): Record<string, any> {
  ...
  const summary = {
    ...
    averageResponseTime:
      Object.values(metrics).reduce((sum, m) => sum + m.avg, 0) /
        Object.keys(metrics).length || 0,
    slowRequests: Object.entries(metrics).filter(([_, m]) => m.avg > 1000)
      .length,
    failedRequests: Object.keys(metrics).filter(key => key.includes('_error'))
      .length,
