...
// WATI Configuration
const WATI_API_URL =
  process.env.WATI_API_URL ?? 'https://live-mt-server.wati.io/320631';
const WATI_ACCESS_TOKEN = process.env.WATI_ACCESS_TOKEN ?? '';
const TEMPLATE_NAMESPACE =
  process.env.WATI_TEMPLATE_NAMESPACE ?? 'bc58e840_9936_490d_8bf4_8935dc18adf9';
...
class WhatsAppOTPService {
  ...
  /**
   * Format phone number for WhatsApp (ensure proper country code with + sign)
   */
  private formatPhoneNumber(phone: string): string {
    // Remove all non-digits first to analyze the number
  ...
  private async sendOTPTemplate(
    ...
    try {
      ...
      const parameters = [{ name: '1', value: otp }];
      ...
      if (!response.ok || result.result === false) {
        throw new Error(
          `WATI Template API error: ${result.info ?? result.message ?? response.statusText}`
        );
      }
    ...
    try {
      ...
      if (!this.isValidPhone(phone)) {
        return {
          success: false,
          error:
            'Invalid phone number. Please enter a valid Indian (+91) or Vietnamese (+84) mobile number.',
        };
      }
    ...
    try {
      ...
      if (user.status !== 'APPROVED') {
        return {
          success: false,
          error:
            'Your account is pending approval. Please contact your university administrator.',
        };
      }
