...
import { ShoppingCart, X, Plus, Minus, ChevronUp, IndianRupee } from 'lucide-react'
...
export default function FloatingCartButton({ className = '' }: FloatingCartButtonProps) {
  ...
  const { items, getTotalItems, getTotalPrice, updateQuantity, removeItem, isLoading } = useCart()
  ...
  const handleQuantityChange = async (itemId: string, newQuantity: number, variantId?: string) => {
      ...
      {isExpanded && (
        <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 mb-3 max-h-80 overflow-hidden">
          ...
          <div className="max-h-48 overflow-y-auto">
            {items.map((item) => (
              <div key={`${item.id}-${item.variantId || ''}`} className="p-4 border-b border-gray-50 last:border-0">
