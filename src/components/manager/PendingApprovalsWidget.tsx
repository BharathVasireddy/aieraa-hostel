...
const PendingApprovalsWidget = memo<PendingApprovalsWidgetProps>(({ pendingCount = 0 }) => {
  const { user } = useUser()
  ...
  const fetchPendingStudents = useCallback(async () => {
    ...
    try {
      setLoading(true)
      const response = await fetch(`/api/manager/users?role=STUDENT&status=PENDING&limit=5`)
  ...
  const updateStudentStatus = async (studentId: string, status: 'APPROVED' | 'REJECTED', reason?: string) => {
    try {
  ...
  const handleReject = (student: PendingStudent) => {
    if (confirm(`Are you sure you want to reject ${student.name}'s application?`)) {
      updateStudentStatus(student.id, 'REJECTED', 'Application rejected by manager')
    }
  }
        ...
        <div className="space-y-4">
          ...
          {localPendingCount > pendingStudents.length && (
            <div className="text-center py-2">
              ...
              >
                <span>View {localPendingCount - pendingStudents.length} more pending</span>
                <ArrowRight className="w-4 h-4" />
              </Link>
            </div>
          )}
