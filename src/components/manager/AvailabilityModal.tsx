...
export default function AvailabilityModal({ menuItem, onClose, onSave }: AvailabilityModalProps) {
  ...
  const validateForm = (): string | null => {
    if (mode === 'recurring') {
      ...
      if (new Date(recurringPattern.startDate) > new Date(recurringPattern.endDate)) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/manager/menu/${menuItem.id}/availability`, {
        method: 'POST',
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Availability Type</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            >
              ...
              <p className="text-xs text-gray-500">All Mondays, Tuesdays, etc.</p>
            </button>
        ...
        <div className="p-6">
          {mode === 'recurring' && (
            <div className="space-y-6">
              <div>
                <h4 className="text-lg font-semibold text-gray-900 mb-3">Select Days</h4>
              ...
              {recurringPattern.days.length > 0 && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-sm text-blue-700">
                    ...
                    {recurringPattern.days.map(dayValue => 
                      dayLabels.find(d => d.value === dayValue)?.label
                    ).join(', ')}{' '}
                    from {formatDate(recurringPattern.startDate)} to {formatDate(recurringPattern.endDate)}
                  </p>
                </div>
              )}
            </div>
          ...
          {mode === 'dateRange' && (
            <div className="space-y-6">
              ...
              {dateRange.startDate && dateRange.endDate && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-sm text-blue-700">
                    <strong>Preview:</strong> Item will be available every day from{' '}
          ...
          {mode === 'specificDates' && (
            <div className="space-y-6">
              <div>
                <h4 className="text-lg font-semibold text-gray-900 mb-3">Specific Dates</h4>
                <p className="text-sm text-gray-600 mb-4">
              ...
              <div className="space-y-4">
                ...
                {specificDates.dates.length > 0 && (
                  <div>
                    <h5 className="font-medium text-gray-900 mb-2">Selected Dates:</h5>
                ...
                {specificDates.dates.length > 0 && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <p className="text-sm text-blue-700">
                      <strong>Preview:</strong> Item will be available on {specificDates.dates.length} specific date{specificDates.dates.length > 1 ? 's' : ''}
                    </p>
                  </div>
                )}
              </div>
